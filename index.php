...
<?php
echo "<h1>Прайс лист телефонов:</h1>";
...
foreach ($data as $phone){
	...
	"<p>" .'Категория идентификатора: ' .htmlspecialchars( $phone['categoryId']).
	"<p>". 'Цена: '.number_format($phone['prices']['amount'],2,',',' '). " ".htmlspecialchars( $phone['prices']['curCode']).
	"<p>". 'Производитель: '.htmlspecialchars ($phone['vendor']).
	"<p>". 'Код поставщика: '.htmlspecialchars( $phone['vendorId'] ).
	"<p>" .'Количество: '.htmlspecialchars( $phone['gradeCount']).
	"<p> Ссылка на yandex market:" .'<a href='.htmlspecialchars($phone['link'])."'>";
	echo $phone['link'];
	echo "</a>";
	echo '</div>' ;
}
...
echo "<h1>Производители, у которых рейтинг не меньше четырех:</h1>";
